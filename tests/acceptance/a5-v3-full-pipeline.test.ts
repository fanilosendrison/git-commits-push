// A5 — Full Turnlock v3 CLI pipeline
...
before(() => {
	...
	searchRoot = fs.mkdtempSync(path.join(os.tmpdir(), "a5-v3-pipeline-"));
	...
	const result = spawnSync("sh", ["-c", pipelineCommand], {
		...
		env: {
			...
			PI_SESSION_ID: "a5-v3-full-pipeline",
		},
		encoding: "utf-8",
		timeout: 60_000,
	});
...
describe("A5 — Turnlock v3 full CLI pipeline", () => {
	...
	test("A5-02 | persists and consumes real v3 initial and retry delegations", () => {
		...
		assert.strictEqual("worker" in retryManifest, false);
	});
	...
	test("A5-03 | publishes the mocked LLM plan through the real Git publisher", () => {
		...
		assert.ok(result.stdout.includes("feat: complete v3 pipeline"));
	});
});
