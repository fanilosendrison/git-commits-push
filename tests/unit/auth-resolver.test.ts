...
describe("auth-resolver", () => {
	...
	test("U-AR-02 | Returns token from agent-credentials.json via command execution", async () => {
		fs.writeFileSync(AUTH_JSON_PATH, JSON.stringify({ testprov: "echo static-token-456" }));
		const token = await resolveAuthToken("testprov");
	...
	test("U-AR-04 | Throws if provider absent from ENV and agent-credentials.json", async () => {
		fs.writeFileSync(AUTH_JSON_PATH, JSON.stringify({ otherprov: "echo static-token" }));
		await expect(resolveAuthToken("testprov")).rejects.toThrow("not found in env or agent-credentials.json");
	});

	test("U-AR-05 | Throws an error if dynamic command fails (enforces command strictness)", async () => {
		fs.writeFileSync(AUTH_JSON_PATH, JSON.stringify({ testprov: "sk-static-token-that-is-not-a-command" }));
		await expect(resolveAuthToken("testprov")).rejects.toThrow("Failed to execute credential command");
	});
	...
	test("U-AR-07 | Token is trimmed of whitespace", async () => {
		fs.writeFileSync(AUTH_JSON_PATH, JSON.stringify({ testprov: "echo '  padded-token  \n'" }));
		const token = await resolveAuthToken("testprov");
