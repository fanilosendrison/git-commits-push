...
export class MockTurnlockEnvironment {
	...
	static create(): MockTurnlockEnvironment {
		const runDir = fs.mkdtempSync(path.join(os.tmpdir(), "git-commits-push-tl-run-"));
	...
	writeSettings(settings: Settings): void {
		fs.writeFileSync(path.join(this.runDir, "settings.json"), JSON.stringify(settings, null, 2));
	}
	 ...
	 */
	writeLLMResult(jobId: string, result: CommitJobResult, runId: string = "test-run-seeded"): void {
		const batchDir = path.join(this.runDir, "runs", "git-commits-push-tl", runId, "results", "commit-jobs-0");
		fs.mkdirSync(batchDir, { recursive: true });
		fs.writeFileSync(path.join(batchDir, `${jobId}.json`), JSON.stringify(result, null, 2));
	}
